...
실제 API 서버가 존재하지 않아서 모킹환경을 구성하여 실행
...
> GitHub Actions 또는 로컬 실행 중 선택하여 실행

### GitHub Actions로 실행 (권장)

1. github 저장소 상단 메뉴 Actions 선택
2. 좌측 Actions > API 테스트 (Playwright) 선택
3. 본문 > Run workflow 선택 박스 선택
4. 브랜치 및 테스트 환경 선택 (실제 서버가 없어서 mock 환경 권장)
...
- 실패 시나리오 : 파라미터 누락, 유효하지 않은 형식, 권한이 없는 사용자 요청 등
...
따라서 소프트웨어 개발에서 결함을 조기에 발견하고 수정하는 것이 중요하다는 것을 보여줍니다.
...
* QA 관점
  * 시프트 레프트 전략 - 개발 초기에 QA 활동 강화
    * 요구사항 분석 및 리뷰
      ...
      * 구체적인 사용자 시나리오
  * CI/CD 파이프라인 구축
    * 코드가 커밋될 때마다 정적분석, 단위테스트, 리그레션 테스트 자동화 등이 수행될 수 있도록 구축
